...
NUM_STEPS = 1000
num_steps = NUM_STEPS
at_which_step_random_death = 50 # After every 50 number of steps a death round will be exercised 
probability_death = 0.1 # Almost 10% of pixels and corresponding agencies would die
...
def update_ca(ca_grid, ca_nn_list,frame_number):
            ...
            if high_alpha_pixels:
                ...
                if not has_nonzero_weights_current:
                    ...
                    for name, param in ca_nn_list[idx].named_parameters():
                        ...
                        # <<<<<<<<<<<<<<<<< try bias in mutation routine here <<<<<<<<<<<<<<<<<
                else:
                    ca_nn_list[idx] = copy.deepcopy(selected_nn)
        else:
    ...
    if(frame_number % at_which_step_random_death == 0):
        for x in range(WIDTH):
            for y in range(HEIGHT):
                # Check if any value in channel 0 is less than ALPHA at the current position
