...
budget_per_cell = 7
activation_on_board = ['tanh'] # sigmoid or tanh
...
import csv
# Save data to CSV
with open('ctfp.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # Write header
    header = ['Step'] + [f'Value {key:.{precision}f}' for key in unique_keys]
    writer.writerow(header)
    # Write data rows
    for i in range(len(x_values)):
        row = [x_values[i]] + [y_values[key][i] for key in unique_keys]
        writer.writerow(row)

source_path = "ctfp.csv"
destination_path = 'CSVS'
shutil.move(source_path, destination_path)
...
# Save data to CSV
with open('gep.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # Write header
    writer.writerow(['Step', 'GEP'])
    # Write data rows
    for step, entropy in zip(steps, Global_Entropies_H_ts):
...
# Save data to CSV
with open('gcvp.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # Write header
    writer.writerow(['Step', 'GCVP'])
    # Write data rows
    for step, variance in zip(steps, gross_cell_variance):
        writer.writerow([step, variance])
...
# Save data to CSV
with open('clogv.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # Write header
    writer.writerow(['Step', 'CLOGV'])
    # Write data rows
    for step, sigma in zip(steps, sigma_global):
        writer.writerow([step, sigma])
...
# Save data to CSV
with open('pd_combined.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # Write header
    writer.writerow(['Step', 'GEP', 'GCVP', 'CLOGV'])
    # Write data rows
    for step in range(len(plot_data_tool2)):
        h_t = plot_data_tool2[step] if step < len(plot_data_tool2) else ''
        sigma_gross = plot_data_tool3[step] if step < len(plot_data_tool3) else ''
        sigma_glob = plot_data_tool4[step] if step < len(plot_data_tool4) else ''
        writer.writerow([step, h_t, sigma_gross, sigma_glob])
...
source_path = "gcvp.csv"
destination_path = 'CSVS'
shutil.move(source_path, destination_path)

source_path = "clogv.csv"
destination_path = 'CSVS'
shutil.move(source_path, destination_path)

source_path = "pd_combined.csv"
destination_path = 'CSVS'
shutil.move(source_path, destination_path)
...
# Save data to CSV
with open('rwsp.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # Write header
    writer.writerow(['Step', 'RWSP'])
    # Write data rows
    for generation in range(length):
        count = counts_per_frame[generation]
        writer.writerow([generation, count])
...
# Save data to CSV
with open('ghc.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # Write header
    writer.writerow(['Step', 'GHC'])
    # Write data rows
    for generation in range(length_sim):
        count = counts_per_frame[generation]
        writer.writerow([generation, count])
...
# Save data to CSV
with open('rwsp_ghc.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # Write header
    writer.writerow(['Step', 'RWSP', 'GHC'])
    # Write data rows
    for generation, rwsp_count, ghc_count in zip(generations, counts_rwsp, counts_ghc):
        writer.writerow([generation, rwsp_count, ghc_count])
