...
def save_frame(frame, fig, normalized_data):
    ...
    normalized_colors = unique_colors / 255.0  # Assuming colors are in the range [0, 255]

    # Plot frequency histogram with normalized colors
    bars = ax_freq.bar(range(len(unique_colors)), counts, color=[tuple(color) for color in normalized_colors])
    ax_freq.set_title('Frequency Count')
    ax_freq.set_xlabel('RGB Value')
    ax_freq.set_ylabel('Frequency')
    ax_freq.set_xticks(range(len(unique_colors)))
    ax_freq.set_xticklabels([str(color) for color in unique_colors], rotation=90, fontsize=2)
