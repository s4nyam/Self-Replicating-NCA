...
def save_frame(frame, fig, normalized_data, threshold, counts_at_steps):
    # Create subplots using gridspec
    gs = gridspec.GridSpec(1, 3, width_ratios=[1, 2, 0.5])  # Adjust width ratios as needed
    ax_image = fig.add_subplot(gs[0])  # Image subplot
    ax_freq = fig.add_subplot(gs[1])   # Frequency count subplot

    # Plot the image
    im = ax_image.imshow(normalized_data[frame], cmap='jet')
    ax_image.set_title(f'Generation {frame + 1}')
    fig.colorbar(im, ax=ax_image, orientation='vertical', shrink=0.7)
    ...
    # Filter out bars with count <= threshold
    filtered_unique_colors = unique_colors[counts > threshold]
    filtered_counts = counts[counts > threshold]

    # Normalize colors to [0, 1] range
    normalized_colors = filtered_unique_colors / 255.0  # Assuming colors are in the range [0, 255]

    # Plot frequency histogram with normalized colors
    bars = ax_freq.bar(range(len(filtered_unique_colors)), filtered_counts, color=[tuple(color) for color in normalized_colors])
    ax_freq.set_title('Frequency Count')
    ax_freq.set_xlabel('RGB Value')
    ax_freq.set_ylabel('Frequency')

    # Set xticks and labels
    ax_freq.set_xticks(range(len(filtered_unique_colors)))
    ax_freq.set_xticklabels([str(color) for color in filtered_unique_colors], rotation=90, fontsize=8)
    bars_count = []
    # Annotate bars with counts
    for bar, count in zip(bars, filtered_counts):
        height = bar.get_height()
        bars_count.append(count)
        ax_freq.annotate(f'{count}', xy=(bar.get_x() + bar.get_width() / 2, height),
                         xytext=(0, 3),  # 3 points vertical offset
                         textcoords="offset points",
                         ha='center', va='bottom', fontsize=15)

    counts_at_steps.append(dict(zip(map(tuple, filtered_unique_colors.tolist()), bars_count)))  # Collect counts by color
...
for frame in range(length):
    ...
    save_frame(frame, fig, normalized_data, threshold, counts_at_steps)

# Plot line curves for each color
colors = set()
for counts in counts_at_steps:
    colors.update(counts.keys())

# Define colors for lines based on RGB values of the colors used in the bar plot
color_map = {color: tuple(component / 255.0 for component in color) for color in colors}

# adding new exclusion of dead pixels

# # Calculate the sum of counts for each color over all steps
# color_sums = {color: sum(counts.get(color, 0) for counts in counts_at_steps) for color in colors}

# # Find the color with the highest count sum
# max_count_color = max(color_sums, key=color_sums.get)

# # Plot line curves for each color excluding the color with the highest count sum
# for color in colors:
#     if color != max_count_color:
#         counts_over_time = [counts.get(color, 0) for counts in counts_at_steps]
#         plt.plot(range(1, length + 1), counts_over_time, label=str(color), color=color_map[color])
...
plt.ylabel('Count')
plt.title('Count of Colors Over Time')
# plt.legend(title='Color')
plt.savefig(os.path.join('rwsp_line_curve_plot.png'), format='png', dpi=600)
plt.savefig(os.path.join('rwsp_line_curve_plot.pdf'), format='pdf', dpi=600)
plt.show()
####################################################################################
####################################################################################
####################################################################################
...
source_path = "rwsp_line_curve_plot.pdf"
destination_path = 'GD'
shutil.move(source_path, destination_path)


source_path = "rwsp_line_curve_plot.png"
destination_path = 'GD'
shutil.move(source_path, destination_path)
