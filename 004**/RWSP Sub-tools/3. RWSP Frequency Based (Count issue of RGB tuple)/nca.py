...
def save_frame_with_histogram(frame, normalized_data, length):
    fig, axes = plt.subplots(1, 2, figsize=(19, 7))  # Increase the figure size

    # Plotting the frame
    im = axes[0].imshow(normalized_data[frame], cmap='jet')
    axes[0].set_title(f'Generation {frame + 1}')
    colorbar = fig.colorbar(im, ax=axes[0], orientation='vertical', shrink=0.7)
    ...
    flattened_data = normalized_data[frame].flatten()
    n, bins, patches = axes[1].hist(flattened_data, bins=20, color='skyblue', edgecolor='black')

    # Create a colormap based on the data values
    norm = Normalize(vmin=np.min(flattened_data), vmax=np.max(flattened_data))
    cmap = plt.get_cmap('jet')
    sm = ScalarMappable(norm=norm, cmap=cmap)

    for i, rect in enumerate(patches):
        color = sm.to_rgba((bins[i] + bins[i+1]) / 2)  # Get color corresponding to bin value
    ...
    # Calculate the center of each bin interval for setting ticks
    bin_centers = (bins[:-1] + bins[1:]) / 2

    # Set x-axis ticks at the center of each bin interval
    axes[1].set_xticks(bin_centers)
    axes[1].set_xticklabels([f'{bins[i]:.2f}-{bins[i+1]:.2f}' for i in range(len(bins)-1)], rotation=90)

    fig.tight_layout(rect=[0, 0.03, 1, 0.95])  # Adjust layout to leave space for annotations
...
# Loop through frames and save each
for frame in range(length):
    save_frame_with_histogram(frame, normalized_data, length)
