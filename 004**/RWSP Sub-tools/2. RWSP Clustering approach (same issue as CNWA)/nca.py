...
def save_frame_with_frequency(frame, fig, normalized_data, n_clusters_percentage=0.1):
    # Normalize data
    normalized_frame = normalized_data[frame]

    # Reshape the normalized frame to 2D
    grid_size = normalized_frame.shape[0]
    flattened_grid = normalized_frame.reshape(-1, 3)

    # Determine the number of clusters
    n_clusters = int(len(np.unique(flattened_grid)) * n_clusters_percentage)

    # Group similar colors using clustering
    kmeans = KMeans(n_clusters=n_clusters, random_state=42).fit(flattened_grid)
    unique_labels, counts = np.unique(kmeans.labels_, return_counts=True)

    # Merge similar colors based on cluster centers
    merged_colors = []
    merged_counts = []
    for i, label in enumerate(unique_labels):
        cluster_center = kmeans.cluster_centers_[label]
        if not merged_colors:
            merged_colors.append(cluster_center / 255)  # Normalize colors to [0, 1] range
            merged_counts.append(counts[i])
        else:
            distances = [distance.euclidean(cluster_center, color) for color in merged_colors]
            min_distance = min(distances)
            if min_distance <= 2:  # Threshold distance to merge colors
                index = distances.index(min_distance)
                merged_counts[index] += counts[i]
            else:
                merged_colors.append(cluster_center / 255)  # Normalize colors to [0, 1] range
                merged_counts.append(counts[i])

    # Plot normalized frame
    plt.figure(figsize=(10, 4))  # Create a new figure for each frame
    plt.subplot(1, 2, 1)
    im = plt.imshow(normalized_frame, cmap='jet')
    plt.title(f'Generation {frame + 1}')
    cax = fig.add_axes([0.08, 0.94, 0.15, 0.02])
    colorbar = fig.colorbar(im, cax=cax, orientation='horizontal', shrink=0.7)
    min_value = np.min(normalized_frame)
    max_value = np.max(normalized_frame)
    mid_value = (min_value + max_value) / 2
    ticks = [min_value, (min_value + mid_value) / 2, mid_value, (mid_value + max_value) / 2, max_value]  # Include midpoints
    ...
    # Plot frequency histogram with colors corresponding to the grid cells
    plt.subplot(1, 2, 2)
    bars = plt.bar(range(len(merged_colors)), merged_counts, color=merged_colors)  # Use normalized colors

    plt.title('Frequency Count (Merged Colors)')
    plt.xlabel('Color Group')
    plt.ylabel('Frequency')
    plt.xticks(range(len(merged_colors)), [f'Group {i+1}' for i in range(len(merged_colors))], rotation=90)

    # Annotate bars with corresponding counts
    for i, count in enumerate(merged_counts):
        plt.text(bars[i].get_x() + bars[i].get_width() / 2, count + 0.1, str(count), ha='center')
...
for frame in range(length):
    fig = plt.figure()
    save_frame_with_frequency(frame, fig, normalized_data)
